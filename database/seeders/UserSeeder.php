...
class UserSeeder extends Seeder
{
    ...
    public function run()
    {
            ...
            foreach (Kelas::all() as $key => $kelas) {
                for ($i = 0; $i < $siswa_per_kelas; $i++) {
                    ...
                    DB::table('siswas')->insert([
                        ...
                        'tanggal_keluar' => fake('id_ID')->date('Y-m-d', '+1 years'),
        ...
        $defaultAngkatan = Data_angkatan::first();
        if ($defaultKelas && $defaultAngkatan) {
            $uniqueSuffix = Str::random(3);
            DB::table('siswas')->insert([
                'nis' => random_int(60000, 99999) . '' . random_int(100, 999) . $uniqueSuffix,
                'nisn' => random_int(60000, 99999) . '' . random_int(60000, 99999) . $uniqueSuffix,
                'nik' => random_int(90000, 99999) . '' . random_int(90000, 99999) . '' . random_int(1, 1000) . $uniqueSuffix,
                'nama' => fake('id_ID')->name(),
                'no_telp' => fake('id_ID')->phoneNumber(),
                'nama_ayah' => $ayah = fake('id_ID')->name('male'),
                'nama_ibu' => fake('id_ID')->name('female'),
                'nama_wali' => $ayah,
                'status' => 'bukan pindahan',
                'tanggal_keluar' => now()->addYears(1)->format('Y-m-d'),
                'tanggal_lahir' => fake('id_ID')->date(),
                'tempat_lahir' => fake('id_ID')->citySuffix(),
                'alamat' => fake('id_ID')->city(),
                'jenis_kelamin' => fake('id_ID')->randomElement(['laki-laki', 'perempuan']),
                ...
                'id_angkatan' => $defaultAngkatan->id,
