...
@section('content')
                            ...
                            <tbody>
                                @foreach ($daftar_kelas as $kelas)
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            <span class="badge bg-gradient-primary">{{ $kelas->siswa->count() }}</span>
                                        </td>
                                        <td class="text-center">
                                            <button class="btn btn-link text-primary text-sm p-0" type="button"
                                                data-bs-toggle="modal" data-bs-target="#students-modal"
                                                data-class-name="{{ $kelas->nama_kelas }}"
                                                data-students='@json($kelas->siswa->map(function ($siswa) {
                                                    return [
                                                        'nama' => $siswa->nama,
                                                        'nis' => $siswa->nis,
                                                    ];
                                                }))'
                                                onclick="showStudentsModal(this)">
                                                Lihat Siswa
                                            </button>
                                        </td>
    ...
    <div class="modal fade" id="students-modal" tabindex="-1" aria-labelledby="studentsModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header bg-primary">
                    <h5 class="modal-title text-white" id="studentsModalLabel">Daftar Siswa <span class="text-white-50"></span></h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div id="students-modal-content"></div>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        function showStudentsModal(element) {
            const className = element.getAttribute('data-class-name');
            const studentsRaw = element.getAttribute('data-students');
            const students = studentsRaw ? JSON.parse(studentsRaw) : [];
            const container = document.getElementById('students-modal-content');
            const titleSpan = document.querySelector('#studentsModalLabel span');

            titleSpan.textContent = className ? `(${className})` : '';

            if (!students.length) {
                container.innerHTML = '<p class="text-sm text-muted mb-0">Belum ada siswa aktif pada kelas ini.</p>';
                return;
            }

            const columns = 3;
            const chunkSize = Math.ceil(students.length / columns);
            let html = '<div class="row">';
            for (let i = 0; i < students.length; i += chunkSize) {
                const chunk = students.slice(i, i + chunkSize);
                html += '<div class="col-md-4"><ul class="list-group mb-3">';
                chunk.forEach((student) => {
                    html += `<li class="list-group-item py-2 d-flex justify-content-between align-items-center">
                                <span class="text-sm">${student.nama}</span>
